...
% In addition, to design parameterization of these conditional proposals, which need to appropriately account for the amount of data, we propose a class of networks that we refer to as \emph{neural sufficient statistics}, which define parameterizations of proposals in a manner that is additive in the local variables, much like the sufficient statistics in conjugate-exponential families.
...
% As a means of generating high-quality samplesin an incremental manner, we develop methods that are inspired by EM and classical Gibbs sampling, which perform iterative updates to blocks of variables.
We here assume that the latent variables in the generative model decompose into blocks $\z = \{\z_1, \ldots, \z_B\}$ and train proposals $q_\f(z_b \mid x, z_{-b})$ that update the variables in a each block $z_{b}$ conditioned on the variables in the remaining blocks $\z_{-b} = z \setminus \{z_b\}$.
...
%     q_\f(z^k \mid \x, z^{k-1}).
% \end{equation*}
We want to train proposals that improve the quality of each sample $z^k$ relative to that of the preceding sample $z^{k-1}$. To accomplishing this, we need to learn a sweep update that leaves the target density \emph{invariant}
% \begin{equation}
...
This means a sweep update learned at training time can be applied at test time to iteratively improve sample quality, without requiring a pre-specified number of updates $K$. In addition, when each single block update $q_\f(z'_b \mid x, z_{-b})$ also leaves the target density \emph{invariant},
\begin{align}
...
% which recursively construct a sequence of weights $w^k = v^k w^{k-1}$ by assuming (see Appendix~\ref{appendix:sis-weight} for detailed proof)
% \begin{align*}
%     w^k
%     = 
%     v^k w^{k-1}
...
\begin{align}
    ...
    .
\end{align}
This construction defines a valid importance sampler for \emph{any} choice of proposal kernel $q(z^k \mid z^{k-1})$ and reverse kernel $r(z^{k-1} \mid z^{k})$. For a given choice of proposal, the \emph{optimal} reverse kernel is $r(z^{k-1} \mid  z^k) = p_\q(x,z^{k-1}) q(z^{k} \mid  z^{k-1}) / p_\q(x,z^k)$, which minimizes the variance of the weights $w^k$. We now use the kernel Equation~\ref{eq:approx-gibbs-kernel} as both the forward and the reverse kernel $q(z^{k} \mid  z^{k-1}) = r(z^{k-1} \mid  z^k) = q_\f(z^k \mid x, z^{k-1})$. When it converges to the actual Gibbs kernel, this choice becomes optimal, since the kernel will satisfy detailed balance $p_\q(x, z^k)\,q_\f(z^{k-1} \,|\, x, z^{k}) = p_\q(x, z^{k-1})\,q_\f(z^{k} \,|\, x, z^{k-1})$.
...
%     v
%     = 
%     p_\q(x, z'_b, z_{-b}) q_\f(z_b \mid  x, z_{-b}) \: / \:
...
Finally we consider an unsupervised model for bouncing MNIST~\cite{srivastava2015unsupervised}, learning a deep generative model and an inference model that performs tracking. For all architecture and training details, see Appendix~\ref{appendix:arch-all} .
\begin{figure*}[t!]
  \centering
...
We compare the APG sampler to samples from a encoder with MLP and LSTM architectures trained by RWS. Figure \ref{samples-gmm} shows that RWS encoder fails to propose reasonable clusters, whereas the APG sampler typically converges within 12 iterations across a range of dataset sizes. With the case of GMM where the exact conditional posterior is tractable, we further verify the convergence of the learned proposals by computing the KL $\mathcal{K}_b(\f)$ defined in equation ~\ref{eq:variational_objective} (see Appendix~\ref{appendix:table-both-mm}).
% \begin{figure}[t!]
%   \centering
...
Once again, we compare the APG sampler with the encoders using RWS (see Figure~\ref{samples-dgmm}, Appendix~\ref{appendix:samples-mixture} and Appendix~\ref{appendix:table-both-mm} for results, see Appendix \ref{appendix:arch-dgmm} for architecture and training details) as GMM. The APG sampler scales to a large range of number of variables, whereas a standard encoder trained using RWS fails to produce reasonable proposals.

\textbf{Quantitative Evaluation in Mixture Models}. To assess sample quality, we compute both the log joint $\log p_\q(\x, z)$ and the effective sample size (ESS, see Appendix~\ref{appendix:ess}) w.r.t. each block update. The results (see Appendix~\ref{appendix:table-both-mm}) show that the APG sampler acquires higher log joint and significantly improves ESS. As a mean of comparing the performance of APG samplers for varying numbers of sweeps $K$, we additionally perform an experiment (see Figure~\ref{fig:budget}) in which the computation budget is fixed at $K \cdot L = 1000$ samples. We can see that it is more effective to perform more APG sweeps $K$ with a smaller number of particles $L$, that it is to increase the particle budget. 
\subsection{Time Series Model -- Bouncing MNIST}
Finally, we apply the APG sampler to a time series model that is trained with short timescales, and evaluate it with longer timescales.
...
where  ST is a spatial transformer \cite{jaderberg2015spatial} (see Appendix~\ref{appendix:st-bmnist} for details).At test time, APG sampler can scale up to $T=100$ time steps and $D=5$ digits (see Figure~\ref{mnist-qualitative} and Appendix~\ref{appendix:samples-bmnist}).
...
\subsection{Quantitative Results in Mixture Models}
\label{appendix:table-both-mm}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.8\columnwidth]{figures/bmnist-mse.pdf}
...
\subsection{Spatial Transformer in Bouncing MNIST}
\label{appendix:st-bmnist}
To recap, the deep generative model in bouncing MNIST is 
{\footnotesize
\begin{align*}
    z^{\mathrm{what}}_{d} 
    \sim 
